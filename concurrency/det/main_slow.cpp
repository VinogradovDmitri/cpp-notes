...
ll det_parallel(const vector<int>& rows, const vector<int>& cols, uint8_t max_threads, bool allowed = true) {
    int n = rows.size();
    ...
    for (int i = 0; i < n; ++i) {
        ...
        vector<int> cols_;
        cols_.reserve(n - 1);
        for (int k = 0; k < n; ++k) if (k != i) cols_.push_back(cols[k]);

        if (allowed && t_cnt.load() < max_threads) {
            int cur = t_cnt.load();
            bool created = false;
            while (cur < max_threads) {
                if (t_cnt.compare_exchange_weak(cur, cur + 1)) {
                    threads.emplace_back([&, rows_ = move(rows_), cols_ = move(cols_), i]() {
                        minor[i] = det_parallel(rows_, cols_, max_threads, false);
                        t_cnt.fetch_sub(1);
                    });
                    created = true;
                    break;
...
ll det(const vector<int>& rows, const vector<int>& cols, uint8_t max_threads) {
    return det_parallel(rows, cols, max_threads, true);
}

double run_measure(uint8_t max_threads, const vector<int>& all_rows, const vector<int>& all_cols, bool print = false) {
    auto start = chrono::high_resolution_clock::now();
    ll res = det(all_rows, all_cols, max_threads);
    if (print) cout << res << "\n";
...
int main(int argc, char* argv[]) {
    ...
    uint8_t max_possible = thread::hardware_concurrency();
    ...
    if (quiet) {
        ...
        for (uint8_t threads = 2; threads <= 2*max_possible+1; ++threads) { // нет особого смысла тк матрица максимум 13x13
            double total = 0.0;
